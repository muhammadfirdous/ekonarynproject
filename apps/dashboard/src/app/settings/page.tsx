...
// SettingsPage hosts DashboardLayout, which is the component that gates
// rendering on the /auth/me round-trip. The form lives in a child rendered
// only after `user` is non-null, so its useState initializer always sees the
// real user data (instead of running once with user=null and never re-seeding).
type SettingsUser = { id: string; name: string; phone: string; role: string };

function SettingsForm({ user, token }: { user: SettingsUser; token: string | null }) {
  const t = useT();
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        disabled={saving}
        className="inline-flex items-center gap-2 bg-brand-700 text-white px-6 py-2.5 rounded-lg disabled:opacity-50"
      >
